...
class LBFGS(Optimizer):
    ...
    def __init__(self, params, lr=1, history_size=10, line_search='Wolfe', 
                 dtype=torch.float, debug=False,use_constraints=False):
        ...
        defaults = dict(lr=lr, history_size=history_size, line_search=line_search, 
                        dtype=dtype, debug=debug, use_constraints=use_constraints)
    ...
    def _add_update(self, step_size, update,use_constraints):
        offset = 0
        for p in self._params:
            numel = p.numel()
            # view as to avoid deprecated pointwise semantics
            if p.requires_grad:
                update_temp=update[offset:offset + numel].view_as(p.data)*step_size
                if use_constraints:
                    update_temp_c=torch.max(update_temp,-p.data)
                    update_temp_c = torch.min(update_temp_c, 1-p.data)
                    step_size_factor=min(torch.div(update_temp_c,update_temp))
                    if step_size_factor<0.1:
                        update_temp=update_temp_c
                p.data.add_(update_temp)
                #p.clamp_(0, 1)
    ...
    def _step(self, p_k, g_Ok, g_Sk=None, options={}):
        ...
        dtype = group['dtype']
        debug = group['debug']
        use_constraints = group['use_constraints']
        ...
        if(line_search == 'Armijo'):
            ...
            self._add_update(t, d,use_constraints)
            F_new = closure()
            closure_eval += 1
            ...
            while F_new > F_k + c1*t*gtd or not is_legal(F_new):
                ...
                if(ls_step >= max_ls):
                    ...
                    if inplace:
                        self._add_update(t-t_new, d,use_constraints)
                    else:
                        self._load_params(current_params)
                        self._add_update(t, d,use_constraints)
        ...
        # perform weak Wolfe line search
            ...
            while True:
                ...
                # update parameters
                if inplace:
                    self._add_update(t - t_prev, d,use_constraints)
                else:
                    self._load_params(current_params)
                    self._add_update(t, d,use_constraints)
...
class FullBatchLBFGS(LBFGS):
    ...
    def __init__(self, params, lr=1, history_size=10, line_search='Wolfe', 
        ...
        super(FullBatchLBFGS, self).__init__(params, lr, history_size, line_search, 
             dtype, debug,use_constraints=use_constraints)
